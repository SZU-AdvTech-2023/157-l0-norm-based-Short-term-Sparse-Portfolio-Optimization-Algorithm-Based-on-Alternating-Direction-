...
if __name__ == '__main__':
    ...
    # K_value = [3, 5, 7]
    # for k in K_value:
    #     result = tools.quickrun(SSPO_L0(K=k, C=10, lamda=0.1, gamma=1, rho=10), tools.dataset(datasetName))
    #     file_path = "D:\SZU_homework\portfolios\\universal-portfolios-master\\algo_result\sspo_l0_K%d-tse.xlsx" %(k)
    #     tools.result_saver(result, file_path)

    result = tools.quickrun(SSPO_L0(K=3, C=10, lamda=0.1, gamma=1, rho=10), tools.dataset(datasetName))
    file_path = "D:\SZU_homework\portfolios\\universal-portfolios-master\\algo_result_2\sspo_l0_K3-sp500.xlsx"
    tools.result_saver(result, file_path)