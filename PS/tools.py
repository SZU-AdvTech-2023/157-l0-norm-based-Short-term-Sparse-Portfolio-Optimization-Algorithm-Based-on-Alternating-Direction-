...
def result_saver(algo_result, file_path):
    file_path = file_path


    B = algo_result.B
    asset_equity = algo_result.asset_equity
    r = algo_result.r
    benchmark_r = algo_result.benchmark_r
    weights = algo_result.weights
    cw = algo_result.cw

    summary = pandas.DataFrame({"profit factor":[algo_result.profit_factor],
                                "sharpe":[algo_result.sharpe],
                                "sharpe_std":[algo_result.sharpe_std],
                                "ulcer index":[algo_result.ulcer],
                                "information ratio (wrt benchmark)":[algo_result.information],
                                "benchmark_sharpe":[algo_result.benchmark_sharpe],
                                "benchmark_sharpe_std":[algo_result.benchmark_sharpe_std],
                                "appraisal_benchmark":[algo_result.appraisal_benchmark],
                                "appraisal_benchmark_std":[algo_result.appraisal_benchmark_std],
                                "beta":[algo_result.beta],
                                "alpha":[algo_result.alpha],
                                "Annualized return":[algo_result.annualized_return],
                                "Annualized volatility":[algo_result.annualized_volatility],
                                "Longest drawdown":[algo_result.drawdown_period],
                                "Max drawdown":[algo_result.max_drawdown],
                                "Winning days":[algo_result.winning_pct],
                                "Annual turnover":[algo_result.turnover],
                                "total wealth":[algo_result.total_wealth]
                                })

    with pandas.ExcelWriter(file_path) as file:
        summary.to_excel(file,sheet_name="summary", index=False)
        B.to_excel(file, sheet_name = "B", index = False)
        asset_equity.to_excel(file, sheet_name = "asset_equity", index = False)
        r.to_excel(file, sheet_name="r", index=False)
        benchmark_r.to_excel(file, sheet_name="benchmark_r", index=False)
        weights.to_excel(file, sheet_name="weights", index=False)
        cw.to_excel(file, sheet_name="cw", index=False)


        # asset_equity.to_excel(file, sheet_name="B", index=False)
    print("结果已保存到文件夹")